...
wykonaj_integracje <- function(kontrast) {
  ...
  rna_sig <- rna %>% 
    ...
    select(Name = RNA, logFC_RNA = logFC, FDR_RNA = adj.P.Val) %>%
    mutate(JoinKey = tolower(Name)) # Klucz do łączenia (małe litery)

  # BIAŁKA: FDR < 0.05
  prot_sig <- prot %>% 
    filter(adj.P.Val < 0.05) %>% 
    select(Name = Protein, logFC_Prot = logFC, FDR_Prot = adj.P.Val) %>%
    mutate(JoinKey = tolower(Name))

  # miRNA: FDR < 0.05, |logFC| > 0.5
  mirna_sig <- mirna %>% 
    filter(adj.P.Val < 0.05 & abs(logFC) > 0.5) %>% 
    select(miRNA, logFC_miRNA = logFC, FDR_miRNA = adj.P.Val)
  ...
  zgodnosc <- rna_sig %>%
    ...
    select(Gen = Name.x, Bialko = Name.y, logFC_RNA, logFC_Prot) %>%
  ...
  targety <- bind_rows(
    rna_sig %>% select(Target = Name, logFC_Target = logFC_RNA, Type = JoinKey) %>% mutate(Source="RNA"),
    prot_sig %>% select(Target = Name, logFC_Target = logFC_Prot, Type = JoinKey) %>% mutate(Source="Protein")
  )
