...
W **modelu bazowym**, uwzględniającym wiek pacjenta, stadium choroby oraz podtyp PAM50, najważniejszymi predyktorami przeżycia okazują się **wiek oraz stadium nowotworu**. Każdy dodatkowy rok życia zwiększa ryzyko zgonu o około 5%, a pacjenci ze stadium II i III mają odpowiednio około 2- i 4-krotnie wyższe ryzyko w porównaniu ze stadium I. Podtyp molekularny PAM50 w tym modelu nie jest istotnym prognostycznie czynnikiem, choć w biologicznej interpretacji pozostaje istotny dla charakterystyki guza. Model bazowy dobrze rozróżnia pacjentów o różnych poziomach ryzyka, co potwierdza wartość współczynnika zgodności (Concordance = 0.743).

Włączenie **miR-210** do modelu (model C1) nie zmienia istotnie wyników w całej kohorcie. Współczynnik hazardu dla miR-210 sugeruje pewien efekt ochronny – wyższa ekspresja może wiązać się z niższym ryzykiem – jednak efekt ten nie jest statystycznie istotny (p = 0.29). Pozostałe zmienne kliniczne zachowują swoje znaczenie, a zdolność predykcyjna modelu nie ulega praktycznej poprawie (Concordance = 0.744).

Podobnie, w modelu C2 z **miR-17**, kierunek efektu wskazuje, że wyższa ekspresja może zwiększać ryzyko zgonu, ale efekt również nie jest istotny statystycznie (HR ≈ 1.12, p = 0.40). Zdolność predykcyjna modelu pozostaje zbliżona do modelu bazowego (Concordance = 0.742).

Podsumowując, w analizie całej kohorty najbardziej prognostycznymi zmiennymi pozostają **wiek i stadium choroby**, natomiast pojedyncze miRNA (miR-210 i miR-17) oraz podtypy PAM50 nie dodają istotnej informacji prognostycznej. Wyniki te uzasadniają zastosowanie podejścia LASSO Cox w celu zbudowania **sygnatury miRNA**, w której łączny efekt kilku markerów może ujawnić silniejsze predykcyjne właściwości, niewidoczne przy analizie pojedynczych miRNA.
...
W **podtypie Luminal A**, wiek pozostaje istotnym czynnikiem prognostycznym – każdy dodatkowy rok zwiększa ryzyko zgonu o około 5,5% (HR = 1.055, p < 0.001). Natomiast ekspresja **miR-210** nie wykazuje istotnego wpływu na przeżycie (HR = 0.93, p = 0.66). Model dobrze rozróżnia pacjentów pod względem ryzyka (Concordance = 0.788), co sugeruje, że w tym podtypie głównym determinantem jest wiek pacjenta, a miR-210 nie wnosi dodatkowej informacji prognostycznej.

W **podtypie Luminal B**, zarówno wiek (HR = 1.027, p = 0.12), jak i miR-210 (HR = 0.76, p = 0.26) nie osiągają istotności statystycznej. Model w tym przypadku ma słabszą zdolność rozróżniania pacjentów (Concordance = 0.606), co może wynikać z mniejszej liczebności kohorty i ograniczonej liczby zdarzeń. Ekspresja miR-210 kierunkowo sugeruje efekt ochronny, ale brak istotności uniemożliwia wnioski kliniczne.

W **podtypie HER2**, wiek ponownie wykazuje znaczenie prognostyczne (HR = 1.072, p = 0.008), natomiast wpływ miR-210 jest nieistotny (HR = 0.71, p = 0.38). Model ma umiarkowaną zdolność predykcyjną (Concordance = 0.726), co wskazuje, że w tym podtypie również głównymi determinantami pozostają czynniki kliniczne, a miR-210 nie dodaje znaczącej informacji.

W **podtypie Basal-like**, ani wiek (HR = 1.018, p = 0.33), ani miR-210 (HR = 1.045, p = 0.90) nie są istotnymi predyktorami przeżycia. Model nie rozróżnia dobrze pacjentów pod względem ryzyka (Concordance = 0.564). Wyniki te sugerują, że w podtypie Basal-like inne czynniki molekularne lub kliniczne mogą odgrywać kluczową rolę w predykcji przeżycia, a miR-210 nie jest użytecznym markerem prognostycznym.
...
W **podtypie Luminal A**, zarówno wiek, jak i ekspresja **miR-17** mają znaczenie prognostyczne. Każdy dodatkowy rok zwiększa ryzyko zgonu o około 5,9% (HR = 1.059, p < 0.001). Ekspresja miR-17 istotnie zwiększa ryzyko – pacjenci z wyższą ekspresją mają o około 66% większe ryzyko zgonu (HR = 1.665, p = 0.048). Model dobrze rozróżnia pacjentów pod względem ryzyka (Concordance = 0.772), co sugeruje, że miR-17 może być wartościowym markerem prognostycznym w tym podtypie.

W **podtypie Luminal B**, ani wiek (HR = 1.028, p = 0.12), ani miR-17 (HR = 0.78, p = 0.39) nie wykazują istotności statystycznej. Model ma słabszą zdolność predykcyjną (Concordance = 0.589), co może wynikać z mniejszej liczby pacjentów i ograniczonej liczby zdarzeń. Ekspresja miR-17 w tym podtypie kierunkowo sugeruje efekt ochronny, ale brak istotności uniemożliwia wnioski kliniczne.

W **podtypie HER2**, wiek pozostaje istotnym predyktorem przeżycia (HR = 1.079, p = 0.003), natomiast wpływ miR-17 jest nieistotny (HR = 0.76, p = 0.56). Model ma umiarkowaną zdolność predykcyjną (Concordance = 0.731). Wyniki sugerują, że w podtypie HER2 główną rolę w prognostyce odgrywają czynniki kliniczne, a miR-17 nie dostarcza dodatkowej informacji.

W **podtypie Basal-like**, ani wiek (HR = 1.020, p = 0.30), ani miR-17 (HR = 1.18, p = 0.52) nie są istotnymi predyktorami przeżycia. Model nie rozróżnia pacjentów pod względem ryzyka (Concordance = 0.564). Podobnie jak w przypadku miR-210, w tym podtypie czynniki molekularne lub kliniczne inne niż badany miR-17 mogą mieć kluczowe znaczenie dla prognostyki.
...
Analiza LASSO-Cox przeprowadzona w celu wyłonienia prognostycznej sygnatury miRNA pozwoliła zidentyfikować cztery miRNA o istotnym znaczeniu: **hsa_mir_210** (współczynnik = −0.0584), **hsa_mir_17** (0.4410), **hsa_mir_20a** (−0.2353) oraz **hsa_mir_155** (−0.2179). Wartości współczynników zostały użyte do obliczenia indywidualnego **Risk Score** dla każdego pacjenta. W badanej kohorcie (n = 782) Risk Score mieścił się w zakresie od −1.19 do 0.88, z medianą −0.115, co pozwoliło podzielić pacjentów na grupy **High Risk** i **Low Risk** względem mediany.

Analiza przeżycia z użyciem modelu Cox wykazała istotną różnicę między grupami ryzyka: pacjenci w grupie Low Risk mieli znacząco niższe ryzyko zdarzenia w porównaniu do High Risk (**HR = 0.61, 95% CI: 0.41–0.92, p = 0.018**). Wynik ten sugeruje, że opracowana sygnatura miRNA jest niezależnym czynnikiem prognostycznym. C-statystyka modelu wyniosła 0.557, co wskazuje na umiarkowaną zdolność dyskryminacyjną sygnatury.

Ponadto, obecność zarówno miRNA o współczynnikach dodatnich (np. **hsa_mir_17**) jak i ujemnych (np. **hsa_mir_210, hsa_mir_20a, hsa_mir_155**) sugeruje, że sygnatura odzwierciedla złożoną regulację molekularną istotną dla przeżycia pacjentów. Sygnatura ta może stanowić podstawę do dalszych badań funkcjonalnych oraz potencjalnie pomóc w identyfikacji pacjentów, którzy mogliby korzystać z bardziej intensywnej terapii lub monitoringu.
