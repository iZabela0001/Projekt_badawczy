...
## Model Sygnatury miRNA (LASSO Cox) 

```{r}
# ==============================================================================
# ETAP 4: MODEL SYGNATURY miRNA (LASSO COX) Z WALIDACJĄ
# ==============================================================================
...
# ------------------------------------------------------------------------------
# KROK 1: Przygotowanie Danych i Automatyczna Selekcja Zmiennych
# ------------------------------------------------------------------------------
...
cat("Całkowita liczba pacjentów (przed podziałem):", nrow(data_model), "\n")

# ------------------------------------------------------------------------------
# KROK 2: Podział na Zbiór Treningowy i Testowy (Train/Test Split)
# ------------------------------------------------------------------------------
set.seed(123) 
...
cat("Liczba pacjentów w zbiorze TRENINGOWYM:", nrow(data_train), "\n")
cat("Liczba pacjentów w zbiorze TESTOWYM:", nrow(data_test), "\n")

# ------------------------------------------------------------------------------
# KROK 3: Trening Modelu LASSO (Tylko na zbiorze TRENINGOWYM)
# ------------------------------------------------------------------------------
# X_train = Macierz markerów dla treningu
x_train <- as.matrix(data_train[, valid_mirna])
# Y_train = Survival object dla treningu
...
# Wykres błędu
plot(cv_fit)
title("LASSO Cross-Validation (Zbiór Treningowy)", line = 3)
...
# ------------------------------------------------------------------------------
# KROK 4: Ekstrakcja Sygnatury (Wagi zmiennych)
# ------------------------------------------------------------------------------
...
if(nrow(signature_table) == 0) warning("LASSO wyzerowało wszystkie zmienne! Spróbuj lambda.1se lub sprawdź dane.")

# ------------------------------------------------------------------------------
# KROK 5: Walidacja na zbiorze TESTOWYM (Egzamin modelu)
# ------------------------------------------------------------------------------
# Uwaga: Tutaj używamy "data_test"! Model nigdy wcześniej nie widział tych danych.

# Przygotowanie macierzy testowej
x_test <- as.matrix(data_test[, valid_mirna])

# Obliczenie Risk Score dla pacjentów testowych używając modelu z treningu
risk_scores_test <- predict(cv_fit, newx = x_test, s = "lambda.min", type = "link")
data_test$RiskScore <- as.vector(risk_scores_test)

# Definiujemy punkt odcięcia (Cutoff) na podstawie MEDIANY ZBIORU TRENINGOWEGO
# (To najbardziej rygorystyczne podejście - zasady ustalamy w treningu)
train_risk_scores <- predict(cv_fit, newx = x_train, s = "lambda.min", type = "link")
cutoff_value <- median(train_risk_scores)

# Klasyfikacja pacjentów testowych
data_test$RiskGroup <- ifelse(data_test$RiskScore > cutoff_value, "High Risk", "Low Risk")

cat("\n--- Statystyki Walidacji (Zbiór Testowy) ---\n")
cat("Punkt odcięcia (z treningu):", round(cutoff_value, 3), "\n")
table(data_test$RiskGroup)

# ------------------------------------------------------------------------------
# KROK 6: Wykres Kaplana-Meiera (Na zbiorze TESTOWYM)
# ------------------------------------------------------------------------------
# Ten wykres pokazuje prawdziwą zdolność predykcyjną modelu
...
p_val_plot <- ggsurvplot(
  ...
  risk.table = TRUE,
  palette = c("red", "blue"),
  title = "Walidacja Sygnatury miRNA (Zbiór Testowy)",
  subtitle = paste("N =", nrow(data_test), "| Split 70/30"),
  xlab = "Time (Days)",
...
print(p_val_plot)
# ggsave("KM_Validation_TestSet.png", print(p_val_plot), width = 8, height = 6)

# ------------------------------------------------------------------------------
# KROK 7: Cox Proportional Hazards dla Ryzyka (Zbiór Testowy)
# ------------------------------------------------------------------------------
cox_test <- coxph(Surv(time_to_event, event) ~ RiskGroup, data = data_test)
cat("\n--- Wynik Coxa dla Grupy Ryzyka (Walidacja) ---\n")
print(summary(cox_test))

# Opcjonalnie: C-index na zbiorze testowym
c_index <- summary(cox_test)$concordance
cat("\nC-index na zbiorze testowym:", round(c_index[1], 3), "\n")
```
